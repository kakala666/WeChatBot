...
from scr.douBao import history
...
__friends_NtoU_dict = {friend["nickname"]: friend["user_id"] for friend in QQcof["ALLFRIENDDATA"]}
__friends_UtoN_dict = {friend["user_id"]: friend["nickname"] for friend in QQcof["ALLFRIENDDATA"]}
__group_NtoU_dict = {friend["group_name"]: friend["group_id"] for friend in QQcof["ALLGROUPDATA"]}
__group_UtoN_dict = {friend["group_id"]: friend["group_name"] for friend in QQcof["ALLGROUPDATA"]}
def isStart():
    try:
        re=requests.get(headers=headers,url=url+"get_status").json()
    except Exception as e:
...
#用户名QQ号互转
def name_id_swapping(data:Union[str,int]):
    if isinstance(data,int):
        return __friends_UtoN_dict.get(data)
    elif isinstance(data,str):
        return __friends_NtoU_dict.get(data)

def groupName_ID_swapping(data:Union[str,int]):
    if isinstance(data,int):
        return __group_UtoN_dict.get(data)
    elif isinstance(data,str):
        return __group_NtoU_dict.get(data)

async def send_private_msg(message,user_id = None):
    URL = f"http://localhost:{cof['QQAPIPort']}/send_msg/"
    msg = {
        "user_id" : user_id,
        "message" : message
...
async def get_friend_list():
    ...
    if isStart():
        data = requests.get(url + "get_friend_list").json()["data"]
        friendNameList = []
        friendUserIdList = []
        for i in data:
            friendNameList.append(i["nickname"])
            friendUserIdList.append(i["user_id"])
        __update_cache("ALLFRIENDDATA",data)
        return friendNameList, friendUserIdList
    else:
        with open('../QQfriend.json', encoding="utf-8") as f:
            QQcof = json.load(f)["ALLFRIENDDATA"]
...
async def get_group_list():
    if isStart():
        ...
        for i in data:
            groupNameList.append(i["group_name"])
            groupIDList.append(i["group_id"])
        __update_cache("ALLGROUPDATA",data)
        return groupNameList, groupIDList
    else:
        with open('../QQfriend.json', encoding="utf-8") as f:
            QQcof = json.load(f)["ALLGROUPDATA"]
        ...
        for i in QQcof:
            ...
            groupIDList.append(i["group_id"])
        return groupNameList, groupIDList
async def async_callback(message,AItype=cof["Bot"]):
    # print(message)
    MySignal.getPrivateMessage.message_received.emit(message)
    msg = json.loads(message)
    #如果接收到的是私聊消息
    if msg.get("post_type") == "message" and msg.get("message_type") == 'private':
        #检测接收到的QQ号是否合法
        if not checkQQ(msg.get("user_id")):
            return
        #检测QQcof是否存在
        if not QQcof:
            return
        else:
            #检验QQcof[Private]是否存在
            if "Private" not in QQcof:
                return
            else:
                #如果user_id不在QQcof[Private]中，终止操作
                if msg.get("user_id") not in QQcof["Private"]:
                    return
        # 构建AI消息结构
        sendMsg = {}
        sendMsg["sub_type"] = "private"
        sendMsg["user_id"] = msg.get("user_id")
        sendMsg["user_name"] = msg.get("sender").get("nickname")
        sendMsg["message"] = msg.get("message")[0].get("data").get("text")
        sendMsg = json.dumps(sendMsg,ensure_ascii=False)
        AIchat = AIPool.get(msg.get("user_id"))
        try:
            if AIchat:
                reText = AIchat.chat(sendMsg)
                print(reText)
                asyncio.create_task(send_private_msg(message=reText, user_id=msg.get("user_id")))
            else:
                if AItype == "kimi":
                    ...
                    asyncio.create_task(send_private_msg(message="Bot配置错误", user_id=msg.get("user_id")))
                reText = AIPool[msg.get("user_id")].chat(sendMsg)
                print(reText)
                asyncio.create_task(send_private_msg(message=reText, user_id=msg.get("user_id")))
        except RateLimitError as e:
            asyncio.create_task(send_private_msg(message="发的太快啦，休息一会吧", user_id=msg.get("user_id")))
            print("速率限制，请稍后再试")
        except Exception as e:
            asyncio.create_task(send_private_msg(message="出现未知错误",user_id=msg.get("user_id")))
    #如果接收到的是群消息
        ...
        try:
            if AIchat:
                reText = AIchat.chat(sendMsg)
                print(reText)
                asyncio.create_task(send_private_msg(message=reText, user_id=msg.get("group_id")))
            else:
                if AItype == "kimi":
                    AIPool[msg.get("group_id")] = kimi.chat()
                elif AItype == "douBao":
                    ...
                    asyncio.create_task(send_private_msg(message="Bot配置错误", user_id=msg.get("group_id")))
                reText = AIPool[msg.get("group_id")].chat(sendMsg)
                print(reText)
                asyncio.create_task(send_private_msg(message=reText, user_id=msg.get("group_id")))
        except RateLimitError as e:
            asyncio.create_task(send_private_msg(message="发的太快啦，休息一会吧", user_id=msg.get("group_id")))
            print("速率限制，请稍后再试")
        except Exception as e:
            asyncio.create_task(send_private_msg(message="出现未知错误", user_id=msg.get("group_id")))

async def listen_to_server(port):
    uri = f"ws://localhost:{port}"
    async with websockets.connect(uri) as websocket:
        while True:
            message = await websocket.recv()
...
def run_async_function(port):
    asyncio.set_event_loop(asyncio.new_event_loop())
    asyncio.get_event_loop().run_until_complete(listen_to_server(port))

def run(port):
    thread = threading.Thread(target=run_async_function, args=(port,))
    thread.start()"""
