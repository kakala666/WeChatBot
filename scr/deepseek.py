...
class chat:
    ...
    def split_string(input_string):
        pass
    def get_think_text(self,text):
        # 按照 </think> 分割字符串
        parts = text.split("</think>")
        if len(parts) < 2:
            return None, text.strip()  # 如果没有找到 </think>，返回 None 和原始字符串
        else:
            # 提取 <think> 和 </think> 之间的内容，并去除多余的空白字符
            first_part = parts[0].replace("<think>", "").strip()
            # 第二部分是 </think> 之后的内容，去除多余的空白字符
    ...
    def chat(self,query):
        ...
        print('完整回复'+result)
        return self.get_think_text(result)[1]
